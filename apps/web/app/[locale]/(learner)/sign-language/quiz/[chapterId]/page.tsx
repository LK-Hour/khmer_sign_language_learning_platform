...
export default function ChapterQuizPage({ params }: ChapterQuizPageProps): JSX.Element {
  ...
  useEffect(() => {
    async function load(): Promise<void> {
      try {
        const payload = await fetchChapterQuiz(chapterId);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const response = await submitChapterQuiz(chapterId, { score, stars });
      const url = `/sign-language/quiz/${chapterId}/results?score=${response.score}&passed=${
        response.passed ? "1" : "0"
